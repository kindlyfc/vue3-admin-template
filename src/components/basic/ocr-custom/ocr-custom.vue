  ...
  <div class="flex">
    <div>
      ...
      <div class="btn_container">
        <a-button type="danger" class="mr20px!" b-rd-6px @click="closeOCR"> 关闭OCR识别 </a-button>
    ...
    <div class="list_container">
      ...
      <div v-if="boxList.length > 0" class="list_body">
        <div class="list_bodaytitle list_th" v-for="(item, index) in boxList" :key="index">
          <div :style="{ color: item.color }"
            ><span style="color: red">*</span> 文本区域{{ index + 1 }}</div
          >
          <!-- <div><a-input placeholder="请输入" v-model:value="item.content"></a-input></div> -->
          <div>
            <a-input placeholder="请输入字段名称"></a-input>
            <!-- <a-tree-select
              ...
              :disabled="index == boxList.length - 1 ? isPending : false"
              placeholder="请选择"
              allow-clear
      ...
      <div class="btn_container1" v-if="boxList.length > 0">
        ...
        <a-button type="primary" b-rd-6px @click="save" :loading="props.loading"> 保存 </a-button>
      </div>
    </div>
  </div>
</template>
