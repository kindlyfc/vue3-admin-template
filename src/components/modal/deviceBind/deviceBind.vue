...
<script lang="ts">
  ...
  import { pointerDevice, pointerBindDevice } from '@/api/alarmList';
  /**
   * 新增名单库弹窗组件
   */
    ...
    setup(props, context) {
      ...
      watch(
        ...
        (val: any) => {
          ...
          if (val && val.deviceInfos?.length) {
            selectedRowKeys.value = val.deviceInfos?.map((v) => {
              return v.id;
            });
            selectedRows.value = val.deviceInfos?.map((v) => v.id);
          }
        },
        {
          immediate: true,
      ...
      const rowSelection: TableProps['rowSelection'] = {
        onChange: (rowKeys: string[], rows: any[]) => {
          ...
          selectedRows.value = rows.map((v) => v.id);
        },
        columnWidth: 70,
      ...
      const getList = async (isReset?: boolean, isPagin?: boolean) => {
        ...
        const data: any = {
          ...
          positionId: props.record.id,
        };
        tableLoading.value = true;
        try {
          const res = await pointerDevice(data);
      ...
      async function submit() {
        ...
        let data: any = {
          id: props.record?.id,
          deviceInfoIds: selectedRows.value || [],
        };
        submitLoading.value = true;
        try {
          await pointerBindDevice(data);
